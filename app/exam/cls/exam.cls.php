...
class exam_exam
{
	...
	//参数：考试设置参数（数组）
	//返回值：插入ID
	public function addExamQuestions($args)
	{
		$args['examtime'] = TIME;
		$data = array('examsquestions',$args);
		$sql = $this->pdosql->makeInsert($data);
		$this->db->exec($sql);
		return $this->db->lastInsertId();
	}
	...
	//参数：考试设置参数（数组）
	//返回值：插入ID
	public function updateExamQuestions($eid,$args)
	{
		$data = array('examsquestions',$args,array(array("AND","eid = :eid",'eid',$eid)));
		$sql = $this->pdosql->makeUpdate($data);
		return $this->db->exec($sql);
	}
